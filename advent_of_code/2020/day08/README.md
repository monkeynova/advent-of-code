...
or `nop`) and an *argument* (a signed number like `+4` or `-20`).
  ...
  * `acc` increases or decreases a single global value called the *accumulator*
    ...
    increase the accumulator by 7. The accumulator starts at `0`. After
    an `acc` instruction, the instruction immediately below it is
    executed next.
...
a `nop` is supposed to be a `jmp`. (No `acc` instructions were harmed
