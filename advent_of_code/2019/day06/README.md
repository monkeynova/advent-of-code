...
is in orbit around exactly one other object. An [orbit](https://en.wikipedia.org/wiki/Orbit)
...
Orbit Map facility uses *orbit count checksums* - the total number of *direct
...
Here, we can count the total number of orbits as follows:
  ...
  * `D` directly orbits `C` and indirectly orbits `B` and `COM`, a
    total of `3` orbits.
