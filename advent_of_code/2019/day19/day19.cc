...
class TractorSearch {
 public:
  ...
  absl::StatusOr<int> ScanRange(Point min, Point max) {
    ...
    for (int y = min.y; y < max.y; ++y) {
      for (int x = min.x; x < max.x; ++x) {
        Point p{x, y};
        ASSIGN_OR_RETURN(bool out, ScanPoint(p));
        VLOG(1) << p << "=" << out;
        if (out) ++active;
      }
    }
  ...
  std::string DebugBoard(Point min, Point max) {
    ...
    CharBoard board(r);
    for (Point p : r) {
      absl::StatusOr<bool> out = ScanPoint(p);
  ...
  absl::StatusOr<Point> FindSquareSpace(int size) {
    ...
    for (Point probe{.x = size, .y = 0};; ++probe.y) {
      ASSIGN_OR_RETURN(bool on, ScanPoint(probe));
      if (on) {
    ...
    while (true) {
      ASSIGN_OR_RETURN(bool on, ScanPoint(ranges.back().min));
      if (!on) {
        ++ranges.back().min.x;
        break;
      }
    ...
    while (true) {
      ASSIGN_OR_RETURN(bool on, ScanPoint(ranges.back().max));
      if (!on) {
        --ranges.back().max.x;
        break;
      }
    ...
    while (true) {
      ...
      ++next_range.max.x;
      while (true) {
        ASSIGN_OR_RETURN(bool on, ScanPoint(next_range.min));
        if (on) break;
        ++next_range.min.x;
      }
  ...
  absl::StatusOr<bool> ScanPoint(Point p) {
    Drone prober(p);
...
static AdventRegisterEntry registry = RegisterAdventDay(
    /*year=*/2019, /*day=*/19,
    []() { return std::unique_ptr<AdventDay>(new Day_2019_19()); });
