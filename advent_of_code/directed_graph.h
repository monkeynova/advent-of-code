...
// std::string_view names. Edges are stored indexed in both directions.
...
class DirectedGraphBase {
 public:
  ...
  void AddNode(std::string_view node_name) { nodes_.insert(node_name); }
  ...
  void AddEdge(std::string_view src, std::string_view dst) {
  ...
  const absl::flat_hash_set<std::string_view>& nodes() const { return nodes_; }

  // Returns all the nodes with an outbound edge from `node`.
  const std::vector<std::string_view>* Outgoing(std::string_view node) const {
  ...
  const std::vector<std::string_view>* Incoming(std::string_view node) const {
  ...
  absl::flat_hash_set<std::string_view> Reachable(std::string_view src) const;

  // Returns the sets of disjoint components of the graph.
  std::vector<std::vector<std::string_view>> Forest() const;
  ...
  // arbitrarily.
  absl::StatusOr<std::vector<std::string_view>> DAGSort() const;
  ...
  // output.
  absl::StatusOr<std::vector<std::string_view>> DAGSort(
      absl::FunctionRef<bool(std::string_view, std::string_view)> cmp) const;

 private:
  absl::flat_hash_set<std::string_view> nodes_;
...
class DirectedGraph : public DirectedGraphBase {
 public:
  ...
  // data.
  void AddNode(std::string_view node_name, Storage s) {
  ...
  const Storage* GetData(std::string_view node) const {
    auto it = storage_.find(node);
    if (it == storage_.end()) return nullptr;
    return &it->second;
  }

  Storage* GetData(std::string_view node) {
    auto it = storage_.find(node);
    if (it == storage_.end()) return nullptr;
    return &it->second;
  }
 ...
 private:
  absl::flat_hash_map<std::string_view, Storage> storage_;
};
...
class Graph : public DirectedGraph<Storage> {
 public:
  void AddEdge(std::string_view a, std::string_view b) {
